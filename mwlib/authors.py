...
class inspect_authors(object):
    ...
    def get_authors(self, revs):
        ...
        changes of given article (before given revision).
        ...
        Authors are sorted by the approximate size of their
        contribution.

        Edits are considered to be reverts if two edits end up in the
        sames size within N edits and have the reverting edit has the
        revid of the reverted edit in its comment.
        ...
        # calculate an approximate size for each edit (true if author only *added* to the article)
        if USE_DIFF_SIZE:
            for i, r in enumerate(revs):
                if i == 0:
                    r['diff_size'] = r['size']
